...
## Edits around line 584 - bring crai/bai files into same directory as cram/bam files. 
##
...
task M2 {
    ...
    command <<<
        ...
        # Additions
        samtools view -H ~{tumor_bam} | cat  
        
        
        ln -s "~{tumor_bam}" "~{Sample}.cram"
        ln -s "~{tumor_bai}" "~{Sample}.cram.crai"
        
        
        gatk --java-options "-Xmx~{command_mem}m" GetSampleName -R ~{ref_fasta} -I ~{tumor_bam} -O tumor_name.txt -encode
        tumor_command_line="-I ~{Sample}.cram -tumor ~{Sample}"
